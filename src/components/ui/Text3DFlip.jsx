  ...
  return (
    ...
    >
      ...
      {characters.map((wordObj, wordIndex) => (
        <span key={wordIndex} className="text-3d-flip-word">
          ...
          {wordObj.needsSpace && <span className="whitespace-pre" style={{ whiteSpace: 'pre' }}> </span>}
        </span>
      ))}
    </ElementTag>
  )
...
const CharBox = memo(
  ({
    ...
    >
      <span
        className={`text-3d-flip-face ${textClassName}`}
        style={{ transform: FRONT_FACE_TRANSFORMS[rotateDirection] }}
      >
        {char}
      </span>
      <span
        className={`text-3d-flip-face-back ${flipTextClassName}`}
